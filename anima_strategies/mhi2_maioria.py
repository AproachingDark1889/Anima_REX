...
def generate_signal(df: pd.DataFrame, params: dict) -> Signal | None:
    """
    MHI2_MAIORIA: analiza las últimas N velas (por defecto 5) y, si la mayoría (threshold por defecto 3)
    son alcistas, devuelve señal CALL para la siguiente vela; si la mayoría son bajistas,
    devuelve PUT. Variante de ventana de conteo extendida.
    ...
    Returns:
        Signal(pair, direction, 'mhi2_maioria') o None si no hay mayoría clara.
    """
    ...
    return Signal(pair=pair, direction=direction, strategy='mhi2_maioria')