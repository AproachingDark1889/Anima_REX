...
MAX_API_WORKERS = 8  # límite duro para no saturar la API
...
def fetch_and_store_ohlcv(
    ...
    try:
        ...
        if broker is None:
            from core.anima_broker import conectar_broker  # importa una sola vez al inicio si prefieres
            cred = cargar_config().get("credenciales", {})
        ...
        workers = min(len(pairs), os.cpu_count() or 4, MAX_API_WORKERS)
        logger.info(f"[OHLCV] ThreadPoolExecutor usando {workers} workers (pares={len(pairs)}, cpu={os.cpu_count()}, techo={MAX_API_WORKERS})")
